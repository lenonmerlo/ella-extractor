...
from parsers.itau_latam_pass import parse_itau_latam_pass as parse_itau_latam_pass_text
...
@router.post("/extract/itau-latam-pass")
async def extract_itau_latam_pass(
    response: Response,
    ...
    logger.info(
        "[extract/itau-latam-pass] filename=%s content_type=%s bytes=%d",
        file.filename,
        file.content_type,
...
@router.post("/parse/itau-latam-pass")
async def parse_itau_latam_pass(
    response: Response,
    file: UploadFile = File(...),
) -> dict[str, Any]:
    response.headers["X-Parser-Version"] = os.getenv("VERSION", "dev")
    ...
    pdf_bytes = await file.read()
    logger.info(
        "[parse/itau-latam-pass] filename=%s content_type=%s bytes=%d",
        file.filename,
        file.content_type,
        len(pdf_bytes) if pdf_bytes else 0,
    )
    ...
    write_text_fixture(filename="itau_latam_pass_reference.txt", raw_text=raw_text, base_dir=base_dir)

    result, warnings, debug = parse_itau_latam_pass_text(raw_text)
