...
from parsers.bradesco_bank_statement import parse_bradesco_bank_statement as parse_bradesco_bank_statement_text
...
@router.post("/parse/bradesco-bank-statement")
async def parse_bradesco_bank_statement(
    response: Response,
    file: UploadFile = File(...),
) -> dict[str, Any]:
    """Recebe um PDF do extrato bancário Bradesco, extrai o texto e retorna dados estruturados."""
    ...
    logger.info(
        "[parse/bradesco-bank-statement] filename=%s content_type=%s bytes=%d",
        file.filename,
        file.content_type,
    ...
    write_text_fixture(filename="bradesco_bank_statement_reference.txt", raw_text=raw_text, base_dir=base_dir)

    result, warnings, debug = parse_bradesco_bank_statement_text(raw_text)
