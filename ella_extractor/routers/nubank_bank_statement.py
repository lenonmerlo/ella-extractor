...
from parsers.nubank_bank_statement import parse_nubank_bank_statement as parse_nubank_bank_statement_text
...
router = APIRouter(tags=["Nubank"])
logger = logging.getLogger("ella-extractor")


@router.post("/parse/nubank-bank-statement")
async def parse_nubank_bank_statement(
    response: Response,
    file: UploadFile = File(...),
) -> dict[str, Any]:
    """Recebe um PDF do extrato bancário Nubank (Conta), extrai o texto e retorna dados estruturados."""
    ...
    logger.info(
        "[parse/nubank-bank-statement] filename=%s content_type=%s bytes=%d",
        file.filename,
        file.content_type,
    ...
    write_text_fixture(filename="nubank_bank_statement_reference.txt", raw_text=raw_text, base_dir=base_dir)

    result, warnings, debug = parse_nubank_bank_statement_text(raw_text)
