...
router = APIRouter(tags=["Santander"])
logger = logging.getLogger("ella-extractor")


@router.post("/parse/santander")
async def parse_santander_invoice(
    response: Response,
    file: UploadFile = File(...),
) -> dict[str, Any]:
    """Recebe um PDF da fatura Santander, extrai texto e retorna dados estruturados."""
    ...
    logger.info(
        "[parse/santander] filename=%s content_type=%s bytes=%d",
        file.filename,
        file.content_type,
    ...
    write_text_fixture(filename="santander_reference.txt", raw_text=raw_text, base_dir=base_dir)

    result = parse_santander(raw_text)
